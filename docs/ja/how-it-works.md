...
`review` と違い実コードを編集します。ディスク上のリポジトリ、またはレビュー直後にそのまま直す場合は
`review` が PR をチェックアウトした worktree です。worktree を自分で作ることはありません。ファイルに触れる
前に、これから編集する場所を確認します — ブランチ違い、または PR 自体のブランチが `main`/`develop` なら
その場で停止します。
...
すべてのプラットフォームが読む同じコマンドファイルに引き継ぐ。ルール・しきい値・重大度をシムで言い直すことは
しません。各プラットフォームへの導入は [インストール](./install.md) を参照してください。
