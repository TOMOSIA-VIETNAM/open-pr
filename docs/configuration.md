...
> [!NOTE]
> Coming from a build that kept `notebooks/review/` in your workspace: nothing is moved for you. To keep what was learned, move it once — `mkdir -p ~/.open-pr-data && mv notebooks/review ~/.open-pr-data/review` — and drop the `notebooks/review/` line from `.gitignore`. Then run `git worktree prune` in each reviewed repo (a moved worktree leaves its registration at the old path) and `/open-pr:clean` to remove the moved worktrees. From a second workspace, move each `notebooks/review/<repo>/` into `~/.open-pr-data/review/` instead: moving the whole directory again nests it as `review/review/`.
