...
`git`、[`jq`](https://jqlang.org/)、および [`gh`](https://cli.github.com/)（GitHub）または [`glab`](https://gitlab.com/gitlab-org/cli)（GitLab）がインストール済み・ログイン済みであること。Windows ではすべて Git Bash 上で動作します（Claude Code は Git for Windows を必須としています）。`jq` は `winget install jqlang.jq` でインストールしてください。Bitbucket には CLI がなく、環境変数 `BITBUCKET_EMAIL` + `BITBUCKET_API_TOKEN` の API token を読みます。いずれの場合もレビューはそのアカウントで投稿されます — 最小権限と確認コマンドは[ベンダーごとのトークン取得](./credentials.md)を参照してください。
