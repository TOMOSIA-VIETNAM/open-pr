...
> [!NOTE]
> Chuyển từ bản cũ để `notebooks/review/` trong workspace: plugin không tự chuyển. Muốn giữ những gì đã học, chuyển một lần — `mkdir -p ~/.open-pr-data && mv notebooks/review ~/.open-pr-data/review` — và xoá dòng `notebooks/review/` trong `.gitignore`. Sau đó chạy `git worktree prune` trong từng repo đã review (worktree bị chuyển đi để lại registration ở đường dẫn cũ) rồi `/open-pr:clean` để xoá các worktree đã chuyển. Từ workspace thứ hai, chuyển từng `notebooks/review/<repo>/` vào `~/.open-pr-data/review/`: chuyển cả thư mục lần nữa sẽ lồng thành `review/review/`.
