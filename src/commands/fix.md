...
unresolved). `<op> data-dir` → `<data>`; `<memory-dir>` = `<data>/<repo>`, wherever this runs from.
Then `cd` into `<repo_dir>` — this command EDITS that repo's files ⇒ works from inside.
...
1. the current branch matches `headRefName` EXACTLY **and its tip prefix-matches "Head SHA"** ⇒ fix
   ...
   `Fix in a fresh worktree (Recommended)` — `<op> checkout` (the worktree lands under `<data>`),
   gates it to "Head SHA"; the user's own branch/tree stays untouched; `cd` into the printed
   worktree, continue there — vs stop-and-checkout yourself, printing:
   ```
