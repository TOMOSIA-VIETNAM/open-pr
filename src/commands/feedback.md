...
`~/.open-pr-data/review/`, its settings or memory.
...
| `$T/open-pr-feedback.md` | same fields, each `### <that field's heading, Step 3>` + content, in the form's order. FORBIDDEN: guessing it from the field ID, or reading the form files — the plugin ships without them |
