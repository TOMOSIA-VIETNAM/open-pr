...
its meaning. FORBIDDEN: that raw text inside a constructed `Bash` call; every `<op>` call uses the
parsed values only.
...
- stored ≠ parsed → STOP before any fetch; state both values + what this URL's own shape indicates;
  ask; WAIT. FORBIDDEN: silently picking one. The confirmed value = `<vendor>` this run, persisted only
