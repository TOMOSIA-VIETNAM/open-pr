...
Yields `<repo_dir>`; pwd is often a workspace of sibling repos, not the repo. WHAT the caller does with
`<repo_dir>` is the caller's rule, not this file's.
...
2. Else `find . -maxdepth 4 -type d -iname "$REPO" 2>&1 | grep -Ev '^\./.*node_modules'`, then
   ...
   - 0 || ≥2 → ask the user to pick or type a path, suggesting the workspace that holds them — a sibling
     repo is invisible from inside another. Unresolvable → STOP:
     ```
