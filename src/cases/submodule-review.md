...
checkout — the worktree sits under `<data>`, and a submodule never gets a memory directory of its own.
...
- memory/templates SHARE the MAIN repo's directory, `<data>/<repo>/` (`<repo>` = from the
  ORIGINAL PR URL). FORBIDDEN: a separate `<data>/<repo-submodule>/` — bootstrap, doctor and
