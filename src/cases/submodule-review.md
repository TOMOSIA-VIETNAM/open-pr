...
checkout — the worktree sits under `~/.open-pr-data/review/<repo>/worktrees/`, and a submodule never gets
...
- memory/templates SHARE the MAIN repo's directory, `~/.open-pr-data/review/<repo>/` (`<repo>` = from the
  ORIGINAL PR URL). FORBIDDEN: a separate `~/.open-pr-data/review/<repo-submodule>/` — bootstrap, doctor and
