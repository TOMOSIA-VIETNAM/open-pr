...
| User config | `.shared` | `git_remote_type` — both commands need it to pick the vendor branch inside `src/bin/open-pr.sh`, so its valid values are exactly what `target` can print: `github`, `gitlab`, `bitbucket`; `output_language` — the language both commands POST in, distinct from `.shared.chat_language` | reconciled per run against the PR URL's own shape (`core/pr-target.md` §2), so a stale value is caught rather than trusted |
