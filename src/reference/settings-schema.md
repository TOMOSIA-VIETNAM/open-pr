...
One file per reviewed repo: `~/.open-pr-data/review/<repo>/settings.json`, 1 node per feature. Reference
...
The `fields` column carries field names and nothing else — what a field means belongs under the
table, so that every token in a cell can be checked against the JSON above:
