...
## MP5 — Cursor: `.cursor-plugin/plugin.json` + `marketplace.json`

- Acceptance:
  - Manifest tối thiểu: `name`, `description`, `version`, `author`.
  - `marketplace.json` ở root repo liệt kê plugin này (schema Cursor, tách biệt hoàn toàn với
...
- Acceptance:
  ...
  - Mỗi platform 1 mục ngắn: lệnh cài **official do chính platform công bố** + lệnh update + link
    docs gốc của platform. Không đưa đường cài local/copy tay làm đường chính; nếu cần cho người
    phát triển plugin thì gom vào 1 mục "dev install" cuối file, nói rõ là dành cho ai.
